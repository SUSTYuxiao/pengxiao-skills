...
| `docs-notes` | 0.3.1 | 项目文档与调查笔记规范接入器：docs 记当前事实，notes 记因果与证据，按主题检索防重复调查；把自包含 DOCS_NOTES.md 装进目标项目，或把自维护约定并入其他 skill（自身设计决策见其 notes/） |
| `reader-first` | 0.1.0 | 组织和审阅回复与报告，让读者无需替作者补齐解释；提供全局交付底线的显式检查与安装入口 |
| `reading-edition` | 0.3.2 | 把技术 Markdown 报告转单文件自包含 HTML 阅读版——受众自适应、结论前置、按需折叠、信息零丢失 |
